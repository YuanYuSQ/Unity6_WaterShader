Shader "Custom/ToonWater_1.0
"
{
    Properties
    {
        // ==========================================
        // 细分曲面 (Tessellation) - 控制水面网格的精细度
        // ==========================================
        [Header(Tessellation)]
        _TessellationUniform ("细分倍数 (Factor)", Range(1, 64)) = 10.2
        _TessellationMinDist ("最高精度距离 (Min Dist)", Float) = 3.0
        _TessellationMaxDist ("细分消退距离 (Max Dist)", Float) = 100.0

        // ==========================================
        // 1. 水体颜色与透明度 (Color & Transparency)
        // ==========================================
        [Header(1. Color)]
        ...
        // ==========================================
        // 2. 菲涅尔与边缘环境泛光 (Fresnel & Environment Glow)
        // ==========================================
        ...
        _GlowReflectionSplit ("泛光与反射分离度 (Split)", Range(0.0, 1.0)) = 0.572
        // 独立的泛光控制
        _FresnelGlowPower ("边缘泛光范围集中度 (Glow Power)", Range(0.1, 50.0)) = 2.0
        _FresnelGlowIntensity ("边缘泛光强度 (Glow Intensity)", Range(0.0, 10.0)) = 1.69

        // ==========================================
        // 3. 表面波纹法线 (Surface Normal)
        // ==========================================
        [Header(3. Normal)]
        [Normal] _NormalMap ("波纹法线贴图 (Normal)", 2D) = "bump" {}
        _NormalScale ("波纹凸起强度 (Scale)", Float) = 1.0
        _NormalSpeed ("波纹流动速度 (Speed XY)", Vector) = (0.0, 0.0, 0, 0)

        // ==========================================
        // 4. 水下折射与色散 (Refraction & Chromatic Aberration)
        // ==========================================
        [Header(4. Distortion)]
        ...
        // ==========================================
        // 7. 顶点物理波浪 (Vertex Gerstner Waves)
        // ==========================================
        [Header(7. Waves)]
        ...
        _CrestFoamThreshold ("浪尖起泡高度阈值 (Crest Foam Thr)", Float) = -0.66
        _CrestFoamStrength ("浪尖白沫浓度 (Crest Foam Str)", Float) = 1.73

        // ==========================================
        // 8. 阳光碎斑与 PBR 透光 (SSS, Glints & PBR Specular)
        // ==========================================
        ...
        _SSSPower ("透光集中度 (SSS Power)", Float) = 3.97
        _SSSDistortion ("透光光线偏折 (SSS Distort)", Range(0, 1)) = 0.062

        [HDR] _GlintColor ("波光碎斑颜色 (Glint Color)", Color) = (1.0, 0.891, 0.692, 1.0)
        _GlintScale ("碎斑密集度 (Glint Scale)", Range(1, 50)) = 1.0
        _GlintThreshold ("碎斑过滤阈值 (Glint Thr)", Range(0.1, 1.0)) = 0.456
        _GlintStrength ("碎斑爆发亮度 (Glint Str)", Range(1, 50)) = 8.5
        _GlintSpeed ("碎斑流动速度 (Glint Speed XY)", Vector) = (0.02, 0.02, 0, 0)

        [HDR] _PBRSpecularColor ("PBR 高光颜色 (GGX Color)", Color) = (1.0, 1.0, 1.0, 1.0)
        _PBRSmoothness ("PBR 高光集中度 (GGX Smoothness)", Range(10.0, 1000.0)) = 10.0
        _PBRSpecularIntensity ("PBR 高光强度 (GGX Intensity)", Range(0.0, 10.0)) = 0.01

        // ==========================================
        // 11. SSR V13 屏幕空间反射 (Mipmap模糊 + 亮度对比度 双通道)
        // ==========================================
        [Header(11. SSR V13 Module (Mipmap Blur  Tone Mapping))]
        _SSRIntensity ("倒影总透明度 (SSR Intensity)", Range(0, 1)) = 0.418
        _SSRMinReflect ("最低反射率 (Min Reflect)", Range(0.0, 1.0)) = 0.143
        _SSRJitter ("倒影噪点抖动 (Jitter)", Range(0.0, 2.0)) = 2.0
        _SSRMaxSteps ("最大步进次数 (Max Steps)", Int) = 20
        _SSRBaseStep ("初始步长 (Base Step)", Float) = 0.23
        _SSRAdaptiveStep ("步长增长率 (Adaptive Step)", Range(0.0, 0.2)) = 0.083
        _SSRBaseThickness ("防穿透容差厚度 (Thickness)", Float) = 0.18

        // [新增] 双通道后处理
        _SSRRoughness ("倒影粗糙度 → Mipmap模糊层级 (Roughness)", Range(0.0, 1.0)) = 0.3
        _SSRBrightness ("倒影亮度 (Brightness)", Range(0.0, 3.0)) = 1.0
        _SSRContrast ("倒影对比度 (Contrast)", Range(0.1, 3.0)) = 1.0

        // 水面穿透截断
        _SSRWaterPlaneBias ("水面穿透容差 (Water Plane Bias)", Range(0.0, 2.0)) = 0.3
        ...
        // 13. TA 调试视图
        // ==========================================
        [Enum(None, 0, SSR_Only, 1, Probe_Only, 2, Mixed, 3, Normal, 4, Foam, 5, Shore_Gradient, 6, Fresnel_Factor, 7, Fresnel_Glow, 8, Fresnel_Glow_Factor, 9, SSR_LOD_Level, 10)] _DebugMode ("Debug view", Float) = 0
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings { float4 positionCS : SV_POSITION; float2 uv : TEXCOORD0; float4 screenPos : TEXCOORD1; float3 positionWS : TEXCOORD2; float3 normalWS : TEXCOORD3; float3 tangentWS : TEXCOORD4; float3 bitangentWS : TEXCOORD5; float crestFoam : TEXCOORD6; };
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float _DeepRange, _WaterAlpha, _BottomRetain, _BottomExposure, _FresnelPower, _NormalScale;
            ...
            half4 frag (Varyings input) : SV_Target
            {
                ...
                // 双层法线混合 —— 两套不同流速/偏移采样相加，产生丰富的微观涟漪
                ...
                // 3. 全向海岸线梯度雷达 (动态白沫)
                // ==========================================
                float2 texelSize = float2(1.0 / _ScreenParams.x, 1.0 / _ScreenParams.y);  // 用标准内置变量替代 _ScreenSize
                ...
                // 岸边泡沫=foamMask(含depthCutoff)，浪尖泡沫=crestFoam×噪声(不受水深限制)
                ...
                for(int i = 0; i < 150; i++)
                {
                    ...
                    {
                        // ==========================================
                        // [V13 双通道后处理] Step 1: Mipmap 空间模糊
                        // 用击中点的屏幕UV，以高 LOD 层级重新采样 _CameraOpaqueTexture，
                        // 利用硬件 Mipmap 链免费获得该点周围的模糊颜色。
                        // roughness=0 → LOD 0 (清晰), roughness=1 → LOD 5 (高度模糊)
                        // ==========================================
                ...
                // ==========================================
                // [V13 双通道后处理] Step 2: 亮度/对比度色调映射
                // 在 Mipmap 模糊之后，再叠加亮度缩放和对比度拉伸，
                // 让倒影在模糊的基础上依然保持可调的视觉冲击力。
                // ==========================================
                if(ssrWeight > 0.001)
                {
                ...
                // 6. 高级物理环境泛光 (重构版)
                // ==========================================
                float fresnelFactor = pow(1.0 - saturate(dot(finalN, viewDir)), _FresnelPower);
                float fresnelWeight = lerp(_SSRMinReflect, 1.0, fresnelFactor);

                // 独立的泛光菲涅尔：用 _FresnelGlowPower 控制边缘集中度，_FresnelGlowIntensity 控制亮度
                float fresnelGlowFactor = pow(1.0 - saturate(dot(finalN, viewDir)), _FresnelGlowPower);
                half3 envGlowColor = probeColor;
                half3 mixedGlowColor = envGlowColor * _FresnelColor.rgb;

                float glowDepthFade = 1.0 - smoothstep(0.0, _DeepRange * 0.5, depthDiffDist);  // 浅水泛光强，深水泛光弱
                ...
                half3 specG = _GlintColor.rgb * smoothstep(_GlintThreshold, _GlintThreshold + 0.05, pow(saturate(dot(finalN, h)), 16.0) * gN) * _GlintStrength * mainLight.color;
                ...
                half3 pbrSpecular = _PBRSpecularColor.rgb * mainLight.color * D * _PBRSpecularIntensity * (1.0 - fresnelFactor);
                ...
                if (_DebugMode == 10.0) return half4(_SSRRoughness.xxx, 1.0);  // [新增] 可视化 Mipmap LOD 层级
